...
def _make_auth_token_factory(
    ...
    *,
    ...
    Resolution order:
      ...
      2. Explicit token command from ``OMNIGENT_DATABRICKS_TOKEN_COMMAND``.
    ...
    # Explicit operator-provided token command (e.g. CoDA's SP OAuth helper).
    # Checked before the host-delegated mint because in header/proxy auth mode
    # the mint endpoint can't issue a token, whereas the command always can.
