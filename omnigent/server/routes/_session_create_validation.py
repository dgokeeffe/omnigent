...
async def validate_existing_host_workspace(
    *,
    ...
    Host access is authorized via the shared-host ``resolve_host_access``
    (owner OR a ``use`` grant OR admin), not owner-only — so a session can be
    created on a host shared with the caller, not just one they own.
    """
    ...
    # host.stat round-trip below. A caller without `use` (owner, a `use`
    # grantee, or admin) must be rejected (403/404 via the shared
