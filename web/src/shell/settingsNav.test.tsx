  ...
  it("includes the Admin group (Members / Policies / Hosts) for any admin, in accounts OR OIDC mode", () => {
    const ids = (accountsEnabled: boolean, isAdmin: boolean) =>
      settingsNavGroups(accountsEnabled, false, isAdmin)
        .flatMap((g) => g.items)
        .map((i) => i.id);
    // Non-admin → no Members / Policies / Hosts, regardless of auth mode.
    expect(ids(true, false)).not.toContain("members");
    expect(ids(false, false)).not.toContain("members");
    expect(ids(true, false)).not.toContain("hosts");
    expect(ids(false, false)).not.toContain("hosts");
    ...
    expect(accountsAdmin?.items.map((i) => i.id)).toEqual(["members", "policies", "hosts"]);
    ...
    expect(oidcAdmin?.items.map((i) => i.id)).toEqual(["members", "policies", "hosts"]);
  });
});
